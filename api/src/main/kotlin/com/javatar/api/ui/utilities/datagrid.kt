...
import tornadofx.Stylesheet.Companion.datagridRow
import java.util.*
...
class DataGrid<T>(items: ObservableList<T>) : Control() {
    ...
    var cellFactory: ((DataGrid<T>) -> DataGridCell<T>)?
        get() = cellFactoryProperty.get();
        set(value) = cellFactoryProperty.set(
            value
        )

    val cellFormatProperty by lazy { SimpleObjectProperty<(DataGridCell<T>.(T) -> Unit)>() }
    var cellFormat: ((DataGridCell<T>).(T) -> Unit)?
        get() = cellFormatProperty.get();
        set(value) = cellFormatProperty.set(
            value
        )
    ...
    val cellWidthProperty: StyleableObjectProperty<Number> = FACTORY.createStyleableNumberProperty(
        this,
        "cellWidth",
        "-fx-cell-width",
        { it.cellWidthProperty },
        150.0
    ) as StyleableObjectProperty<Number>
    var cellWidth: Double
        get() = cellWidthProperty.value as Double;
        set(value) {
            cellWidthProperty.value = value
        }

    val maxCellsInRowProperty: StyleableObjectProperty<Number> = FACTORY.createStyleableNumberProperty(
        this,
        "maxCellsInRow",
        "-fx-max-cells-in-row",
        { it.maxCellsInRowProperty },
        Int.MAX_VALUE
    ) as StyleableObjectProperty<Number>
    var maxCellsInRow: Int
        get() = maxCellsInRowProperty.value.toInt();
        set(value) {
            maxCellsInRowProperty.value = value
        }

    val maxRowsProperty: StyleableObjectProperty<Number> = FACTORY.createStyleableNumberProperty(
        this,
        "maxRows",
        "-fx-max-rows",
        { it.maxRowsProperty },
    ...
    var maxRows: Int
        get() = maxRowsProperty.value.toInt();
        set(value) {
            maxRowsProperty.value = value
        }

    val cellHeightProperty: StyleableObjectProperty<Number> = FACTORY.createStyleableNumberProperty(
        this,
        "cellHeight",
        "-fx-cell-height",
        { it.cellHeightProperty },
        150.0
    ) as StyleableObjectProperty<Number>
    var cellHeight: Double
        get() = cellHeightProperty.value as Double;
        set(value) {
            cellHeightProperty.value = value
        }

    val horizontalCellSpacingProperty: StyleableProperty<Number> = FACTORY.createStyleableNumberProperty(
        this,
        "horizontalCellSpacing",
        "-fx-horizontal-cell-spacing",
        { it.horizontalCellSpacingProperty },
        8.0
    )
    var horizontalCellSpacing: Double
        get() = horizontalCellSpacingProperty.value as Double;
        set(value) {
            horizontalCellSpacingProperty.value = value
        }

    val verticalCellSpacingProperty: StyleableProperty<Number> = FACTORY.createStyleableNumberProperty(
        this,
        "verticalCellSpacing",
        "-fx-vertical-cell-spacing",
        { it.verticalCellSpacingProperty },
        8.0
    )
    var verticalCellSpacing: Double
        get() = verticalCellSpacingProperty.value as Double;
        set(value) {
    ...
    var singleSelect: Boolean
        get() = selectionModel.selectionMode == SINGLE;
        set(value) {
            selectionModel.selectionMode = if (value) SINGLE else MULTIPLE
        }
    var multiSelect: Boolean
        get() = selectionModel.selectionMode == MULTIPLE;
        set(value) {
    ...
    override fun getUserAgentStylesheet(): String =
        tornadofx.DataGrid::class.java.getResource("datagrid.css").toExternalForm()
    ...
    val selectionTriggerScope = SimpleMapProperty<String, (MouseEvent) -> Boolean>(
        FXCollections.observableMap(
            mutableMapOf()
        )
    )
    val selectionActionScope = SimpleMapProperty<String, (T) -> Unit>(FXCollections.observableMap(mutableMapOf()))

    fun selectionTrigger(scopeName: String = "default", trigger: (MouseEvent) -> Boolean) {
        selectionTriggerScope.get()[scopeName] = trigger
    }

    fun onSelectionChange(scopeName: String = "default", action: (T) -> Unit) {
        selectionActionScope.get()[scopeName] = action
    }

    val selectedItem: T? get() = this.selectionModel.selectedItem

    //WTF? idk why this is done like this, it never actually updates the skins.
    ...
    override fun updateItem(item: T?, empty: Boolean) {
        ...
        if (item == null || empty) {
            ...
            if (fresh) {
                ...
                cellFragment =
                    if (cellFragmentType != null) find(cellFragmentType, dataGrid.scope ?: FX.defaultScope) else null
                fresh = false
            }
            cellFragment?.apply {
...
/*class DataGridCellBehavior<T>(private val control: DataGridCell<T>) : CellBehaviorBase<DataGridCell<T>>(control) {
...
class DataGridRowSkin<T>(control: DataGridRow<T>) : CellSkinBase<DataGridRow<T>>(control) {
    ...
    private fun createCell() =
        skinnable.dataGrid.cellFactory?.invoke(skinnable.dataGrid) ?: DataGridCell(skinnable.dataGrid)
...
class DataGridSkin<T>(control: DataGrid<T>) : VirtualContainerBase<DataGrid<T>, DataGridRow<T>>(control) {
    ...
    fun computeMaxCellsInRow(): Int {
        val maxCellsInRow =
            Math.min(Math.max(Math.floor(computeRowWidth() / computeCellWidth()).toInt(), 1), skinnable.maxCellsInRow)

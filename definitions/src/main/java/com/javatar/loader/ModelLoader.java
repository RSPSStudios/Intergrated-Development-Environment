...
import com.javatar.model.ModelPatch;

public class ModelLoader implements DeserializeDefinition<ModelDefinition> {
    ...
    public ModelDefinition load(int modelId, byte[] b) {
        ...
        def.rawModelData = new byte[b.length];
        System.arraycopy(b, 0, def.rawModelData, 0, b.length);
    ...
    private void load1(ModelDefinition model, byte[] var1) {
        ...
        for (point = 0; point < triangleCount; ++point) {
            model.colorModelPatches.add(new ModelPatch(point, var2.getOffset()));
            ...
            if (modelTexture == 1) {
                model.textureModelPatches.add(new ModelPatch(point, var55.getOffset()));
    ...
    private void load2(ModelDefinition model, byte[] var1) {
        ...
        for (var18 = 0; var18 < var11; ++var18) {
            ModelPatch patch = new ModelPatch(var18, var5.getOffset());
            model.colorModelPatches.add(patch);
            ...
            if (var13 == 1) {
                ...
                if ((var8 & 2) == 2) {
                    model.textureModelPatches.add(patch);
    ...
    @Override
    public ModelDefinition deserialize(int id, byte[] b) {
        return load(id, b);
    }
}
